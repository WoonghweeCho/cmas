package com.dwenc.cmas.common.metaData.service;
...
import com.dwenc.cmas.common.naming.DefaultNaming;
...
public class OracleMetaDataService {
    ...
    private DefaultNaming naming = new DefaultNaming();
	...
	public Map<String, Object> getDataList(String dbSpec, String sql) {
		...
		try {
			...
			for (int i = 1; i <= nSize; i++) {
				...
				String attrName = naming.getAttributeName(columnName.toLowerCase());
	...
	public Map<String, Object> getTableDataList(String dbSpec, String searchType, String searchValue, boolean isAllUser) {
		...
		try {
			String owner = StringUtil.getText(appProperties.getProperty("com.dwenc." + dbSpec + ".database.schema")).toUpperCase();
		...
		try {
			...
			while (keys.hasNext()) {
				...
				while (rs.next()) {
					...
					if (inputData != null) {
						String val = StringUtil.getText(inputData.get(naming.getAttributeName(colmn.toLowerCase())));
						if (val != null) {
