...
* @version 1.0
*/
var ds_OuterTripList = new DataSet();
...
function cf_SetComponents()
{
	...
	var outerTripList = {
		datatype: "local",
	   	colNames:[gf_FindLang('출장시작일'),gf_FindLang('일자'),gf_FindLang('소속팀'),gf_FindLang('집행팀'),gf_FindLang('출장자'),gf_FindLang('목적지'),gf_FindLang('출장기간'),gf_FindLang('경비구분'),gf_FindLang('예산내역'),gf_FindLang('결재문서번호'),gf_FindLang('정산서결재문서번호'),gf_FindLang('결재상태'),gf_FindLang('회계상태'),gf_FindLang('정산서'),gf_FindLang('정산구분'),gf_FindLang('REF NO'),gf_FindLang('RPT_REF_NO'),
	   	          gf_FindLang('userId'), gf_FindLang('userNm'),gf_FindLang('docSts'),gf_FindLang('acctSts'),gf_FindLang('ifParam'),gf_FindLang('adjustDocSts'),gf_FindLang('inchrgAir'),gf_FindLang('afare'),gf_FindLang('bustrGl'),gf_FindLang('stayEdYmd'),gf_FindLang('tripUserPositCd'),gf_FindLang('natNm2')
	   	          ],
	   	colModel:[
		   		...
		   		{name:'userIdNm',index:'userIdNm', width:100,align: "center",
		  	   		formatter:function(val, gOpt, row){
		  	   	 		...
		  	   	 		if(row.docSts == "D16" || row.docSts == "D50" || row.docSts == "D60" || row.docSts == "D70"|| row.docSts == "D80"){
		  	   	 			if(row.ifParam == "") return "";
		  	   	 			var data = JSON.parse(row.ifParam);
		   		...
		   		{name:'adjustSignId',index:'adjustSignId', width:120,align: "center", hidden : true},
		   		...
		   		{name:'rptRefNo',index:'refNo', width:100,align: "center",hidden : true},
		   		...
		   		{name:'bustrGl',index:'bustrGl', width:200,align: "center", hidden : true},
		   		{name:'stayEdYmd',index:'stayEdYmd', width:200,align: "center", hidden : true},
		   		{name:'tripUserPositCd',index:'tripUserPositCd', width:150,align: "center", hidden : true},
		   		{name:'natNm2',index:'natNm2', width:150,align: "center", hidden : true}
	   	],
	   	autowidth:true,
	   	height:450,
...
function cf_InitForm()
{
	...
	var	expCls = [
	   	   	...
	   	   	{ "code":"Q1","value":"PJ-현장경비","hgrCommGrpCd":"","hgrCommCd":"","refCd1":"Y","refCd2":"","refCd3":"","refCd4":"" },
	   	   	{ "code":"Q2","value":"PJ-PM경비","hgrCommGrpCd":"","hgrCommCd":"","refCd1":"Y","refCd2":"","refCd3":"","refCd4":"" },
	   	   	{ "code":"Q3","value":"PJ-PPM경비","hgrCommGrpCd":"","hgrCommCd":"","refCd1":"Y","refCd2":"","refCd3":"","refCd4":"" },
	   	   	{ "code":"Q4","value":"PJ-EM경비","hgrCommGrpCd":"","hgrCommCd":"","refCd1":"Y","refCd2":"","refCd3":"","refCd4":"" },
	   	   	{ "code":"Q5","value":"PJ-PCM경비","hgrCommGrpCd":"","hgrCommCd":"","refCd1":"Y","refCd2":"","refCd3":"","refCd4":"" }
	   	   	 ];
...
//	$("#emmisionLayerDiv").css({position:"absolute", top : (offset.top + $(e.target).height()), left:(offset.left + $("#emmisionLayerDiv").width())});
	...
	if(gv_AuthList.auth[0].code.match("RO_CMAS_CO_TRIP_ADM") != null){ //관리권한
		//에러 확인 페이지 오픈
		$("#outerTripError").show();
		// 엑셀버튼
	}
	else if(gv_AuthList.auth[0].code.match("RO_CMAS_TS_ADM") != null){ //관리권한
		$("#excelBtn").show();
	}else{
		// 관리자가 아닌 경우
		cf_RetrieveOuterTripList();
...
function cf_RetrieveOuterTripList(){
	...
	if(gv_AuthList.auth[0].code.match("RO_CMAS_TS_ADM") != null){	//출장멀티조회 권한
		adminYn = "Y";
	}else if(gv_AuthList.auth[0].code.match("RO_CMAS_OT_003")){
		adminYn = "Y";
	}else if(gv_AuthList.auth[0].code.match("RO_CMAS_OT_001")){
		adminYn = "Y";
	}else if(gv_AuthList.auth[0].code.match("RO_CMAS_OT_004")){
		adminYn = "Y";
	}
	...
	gf_Transaction("SELECT_OUTER_TRIP_LIST", "/trip/outerTrip/retrieveOuterTripList.xpl", params, {}, "f_Callback", true);
...
//	var signFormTpCd = rowData.get("signFormTpCd");
	...
	var adjustSignId = rowData.get("adjustSignId");
	...
	// 임시저장일 경우
	if(docSts == "D16" || docSts == "D54" || docSts == "D64" || docSts == "D84"){
		gf_PostOpen("/trip/outerTrip/outerTripDraft.jsp", null,
	...
	}else if(gv_AuthList.auth[0].code.match("RO_CMAS_OT_004") != null && docSts == "D80"){
		// 경영관리팀 검토
		gf_PostOpen("/trip/outerTrip/outerTripDraft.jsp", null,
				"toolbar=no,scrollbars=yes,width=1024,height=700", viewDocParam,
				true, true, "f_RefreshList");

	}else if(docSts == "D50" || docSts == "D60" || docSts == "D70" || docSts == "D80"){
		// 검토중일때의 사용자 View
...
function grd_List_oncelldbclick2(grid, rowid, iRow, iCol, boxid)
{
	...
	var adjustDocSts = rowData.get("adjustDocSts");
	var refNo = rowData.get("refNo");
	var rptRefNo = rowData.get("rptRefNo");
	var signId = rowData.get("signId");
	var adjustSignId = rowData.get("adjustSignId");
	...
	var viewDocParam = {
			...
			rptRefNo     : rptRefNo,
			signId       : signId,
			adjustSignId : adjustSignId,
			cmasList     : ds_CmasList.getAllData()
	};

	if(docSts.substr(0, 1) == "A" || docSts == "D03"){
		// 임시저장일 경우
		if(adjustDocSts == "D16" || adjustDocSts == "D64" || adjustDocSts == "D84"){
		...
		}else if(gv_AuthList.auth[0].code.match("RO_CMAS_OT_004") != null && adjustDocSts == "D80"){
			// 경영관리팀 숙소 검토
			gf_PostOpen("/trip/outerTrip/outerTripAdjust.jsp", null,
					"toolbar=no,scrollbars=yes,width=1024,height=700", viewDocParam,
					true, true, "f_RefreshList");

		}else if(adjustDocSts == "D50" || adjustDocSts == "D60" || adjustDocSts == "D80"){
			// 검토중일때의 사용자 View
...
/** 목록을 엑셀로 다운로드 하는 함수*/
function f_DownExcluExcel() {
	var mappedData = "userKnm:STRING(15):성명," +
					 "tripUserPositCd:STRING(15):호칭," +
					 "orgNm:STRING(40):소속," +
					 "natNm:STRING(20):국가," +
					 "natNm2:STRING(20):국가," +
					 "bustrGl:STRING(50):내용," +
					 "stayStYmd:STRING(20):출장(시작)," +
					 "stayEdYmd:STRING(20):출장(종료)" ;

	var date = new Date().getTime();

	gf_ExportExcel("BusinessTrip_" + date + ".xls", "sheet1", mappedData, ds_OuterTripList);
