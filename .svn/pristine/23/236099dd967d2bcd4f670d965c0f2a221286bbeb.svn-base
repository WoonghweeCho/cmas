/*******************************************************************
 * 01. 업무구분 			: 공통
 * 02. 스크립트 설명  	: 첨부파일 목록 조회 컴포넌트
 * 03. 작성자 			: 변형구
 * 04. 작성일 			: 2013.05.06
 ******************************************************************/

/**
* @class 첨부파일 목록 조회 컴포넌트 class
* @constructor
...
* @version 1.0
*/
uploadlist.prototype.dFormStr = "<form name=\"downForm\" id=\"downForm\" target=\"fileUploader\" method=\"post\" action=\"" + gv_ContextPath + "/co/common/file/downloadUploadedFile.xpl\"></form>";
...
/**
* @see   첨부파일 묶음 번호 hidden field string
* @author 변형구
* @since 2013-05-06
* @version 1.0
*/
...
* @param  aFileAtchId<String> 첨부파일 묶음 번호
...
/**
* @see    첨부파일묶음 번호를 컴포넌트에 setting 함 set과 동시에 조회를 수행한다.
* @param  aFileAtchId<String> 첨부파일 묶음 번호
* @author 변형구
* @since 2013-05-06
* @version 1.0
...
uploadlist.prototype.retrieveFileList = function (aFileAtchId) {
	...
	gf_Transaction("SELECT_FILELIST", "/co/common/file/retrieveWebFileList.xpl", fileInfo, {}, uploadlist.prototype.className+".fileList_Callback", false);
};
...
uploadlist.prototype.retrieveSimpleFileList = function (aFileAtchId) {
	...
	if ( gf_IsNull(aFileAtchId) ) {
		gf_AlertMsg("co.err.nofileatchid");
		return;
	}

	// 조회 transaction 수행
	var fileInfo =  { "fileAtchId":aFileAtchId } ;
	gf_Transaction("SELECT_SIMPLEFILELIST", "/co/common/file/retrieveWebFileList.xpl", fileInfo, {}, uploadlist.prototype.className+".simpleFileList_Callback", false);
};
...
uploadlist.prototype.getList = function(){
	...
	for(var i = 0 ; i < dsFileList.size(); i++){
		tbodyStr += "<tr class=\"f1\" style=\"background-color : #e7ebf7 \">"
			 ...
			 +	"<a style=\"text-decoration:none;\" href='" + gv_ContextPath + "/co/common/file/downloadUploadedFile.xpl?fileAtchId="+dsFileList.get(i, "fileAtchId")+"&fileId="+dsFileList.get(i, "fileId")+"' onclick='"+uploadlist.prototype.className+".downloadFile("+dsFileList.get(i, "fileAtchId")+","+dsFileList.get(i, "fileId")+"); return false;'>" + dsFileList.get(i, "fileNm")
			 + 	"</a>"
			 +"</td>"
