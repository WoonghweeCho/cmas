...
<head>
	<jsp:include page="/common/jsp/comm/include/clientLib.jsp">
		<jsp:param value=":::: 대우건설 시스템 ::::" name="title"/>
		<jsp:param value="frame" name="type"/>
	</jsp:include>
	<ut:script type="text/javascript" src="${contextPath}/js/comm/comcrypto.js"></ut:script>
<style type="text/css">
...
<script type="text/javascript" src="<%=centerUrl%>js/framework/jquery.base64.js"></script>
...
function fcLogin(type) {
	...
	gf_EncryptedLogin(fm.userId.value ,fm.userId.value, $('#f')[0], loclCd);
}
...
function fnLoginCall() {
	var userId = gf_Hangle(loginForm.userId.value);
	var passwd = gf_Hangle(loginForm.userPwd.value);
	$('#j_username').val(userId);
	$('#j_password').val(passwd);

	var frm = $('#f')[0];

	var ifm = gf_GetPoppyFrame()(document);
	frm.target = ifm.name;
	frm.submit();
}
...
<form name='f' id='f' action='${pageContext.request.contextPath}/j_spring_security_check?pt=true' method='POST'>
	...
	<input type="hidden" id="publicKeyModulus" name="publicKeyModulus" value="<%=request.getAttribute("publicKeyModulus")%>">
	<input type="hidden" id="publicKeyExponent" name="publicKeyExponent" value="<%=request.getAttribute("publicKeyExponent")%>">
</form>
