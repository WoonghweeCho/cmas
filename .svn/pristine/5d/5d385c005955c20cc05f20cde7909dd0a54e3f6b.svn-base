...
upload4Rex.prototype.uploadMonitorStr  =
                          ...
                          "		CodeBase=\""+gv_ServerUrl+"/common/install/download/DEXTUploadX.cab#version=3,2,2,0\" VIEWASTEXT> " +
                          "		</OBJECT> ";
...
upload4Rex.prototype.onFileUpload = function () {
	...
	if ( document.all("FileUploadMonitor"+upload4Rex.prototype.dispDivNm).Count > 0 ) {
		...
		// 첨부 묶음번호 전송
		document.all("FileUploadMonitor"+upload4Rex.prototype.dispDivNm).DeleteFormItem("fileAtchId");
...
upload4Rex.prototype.retrieveFileAtchId = function () {
	...
	$.ajax({
	    type: "post",
	    url: gContextPath + '/co/common/file/retrieveFileAtchId.xpl',
	    data: {},
	    datatype: "json",
...
upload4Rex.prototype.uploadCallback = function ( ) {
	...
	// callback 이 수행되는 시점에 컴포넌트 초기화를 수행해준다.
