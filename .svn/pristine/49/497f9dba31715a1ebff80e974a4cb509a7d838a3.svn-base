    ...
    public static String getDetailErrorCode(String errorCause) {
        ...
        try {
            if (errorCause.indexOf("ORA-02292") != -1) {
                ...
                errorCode = "co.err.dwe.nav001"; //duplicate action found (@) for spec (@) \r\n@\r\n@
            } else if (errorCause.indexOf("FRM_MSG_008") != -1) {
                errorCode = "co.err.dwe.msg008"; //Can't find such a Message Code in Message File.
            } else if (errorCause.indexOf("FRM_QRY_006") != -1) {
                errorCode = "co.err.dwe.qry006"; //Can't process xml file(@).
            } else if (errorCause.indexOf("FRM_QRY_007") != -1) {
                errorCode = "co.err.dwe.qry007"; //Can't load the file(@).
            } else if (errorCause.indexOf("FRM_QRY_009") != -1) {
                errorCode = "co.err.dwe.qry009"; //Can't make a query : @ \r\nStatement[@] type=@, condition=@, id=@, value=@, iterate=@, startswith=@, endswith=@, conjunction=@
            }
        } catch (Exception e) {
            return errorCode;
        }
...
//        try {
//            sb.append("\n[").append(Message.getInstance().getMessage(getDetailErrorCode(se))).append("]");
//        } catch (Exception e) {
//            return "";
//        }
//        return sb.toString();
