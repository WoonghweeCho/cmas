...
function cf_SetComponents()
{
	...
	var techDataList = {
		datatype: "local",
		colNames:[gf_FindLang('표제'),gf_FindLang('발행일'),gf_FindLang('발행처'),gf_FindLang('등록번호')],
		   	colModel:[
		   	          {name:'title',index:'title', width:150,align: "left"},
		   	          {name:'issueDd',index:'issueDd', width:50,align: "left"},
		   	          {name:'issueTeam',index:'issueTeam', width:100,align: "left"},
		   	          {name:'disregno',index:'disregno', width:70,align: "center"}
		   	],
...
function cf_RetrieveTechDatList(){
	...
	gf_Transaction("SELECT_TECH_DAT_LIST", "/tech/publication/retrieveTechPubDatList.xpl", params, {}, "f_Callback", true);
