...
function gf_RexReportByDS(aUrl, rexParams, dataSet, buttonPriv, formType, custBtn) {
	...
	if(typeof(dataSet) != "undefined"){
		...
		dataSetNames = function(e){ var r=[]; for(i in e){r[r.length] = e[i].name} return r }(dataSets);
	}
	var rexpertXml = "";
...
function gf_RexConvertPDF(reportPath, rexParams) {
	var url = '';
	if (rexParams) {
		var url = 'co/common/util/ConverterRexXP.xpl?exportFormat=pdf&reportPath=' + reportPath;
		rexParams = gf_RexDefaultParam(rexParams);
		if (rexParams)
			url += '&' + gf_GetUrlWithParam(rexParams);
		//gf_Trace(url);
...
function gf_RptOpenOof(actObj, url, dataSet, rexparams){
	...
	if(typeof(dataSet) != "undefined"){
		...
		dataSetNames = function(e){
								var r=new Array(); for(i in e){r[r.length] = e[i].name} return r
...
// 	sData += "		<connection type='http' namespace='*'>";
// 	sData += "			<config-param-list>";
// 	sData += "				<config-param name='path'><![CDATA[http://localhost:7001/RexServer30/rexservice.jsp]]]]><![CDATA[></config-param>";
// 	//sData += "				<config-param name='cookie'><![CDATA[JSESSIONID=DD02E0FE411A2ECE965972F64ECD9123]]]]><![CDATA[></config-param>";
// 	sData += "			</config-param-list>";
// 	sData += "			<http-param-list>";
// 	sData += "				<http-param name='Q1SQL'>{%auto%}</http-param>";
// 	sData += "				<http-param name='OE'>None</http-param>";
// 	sData += "				<http-param name='CN'>oracle1</http-param>";
// 	sData += "				<http-param name='ID'>SDXML</http-param>";
// 	sData += "				<http-param name='PE'>FALSE</http-param>";
// 	sData += "				<http-param name='QC'>1</http-param>";
// 	sData += "				<http-param name='OT'>DataOnly</http-param>";
// 	sData += "				<http-param name='Q1Type'>SQL</http-param>";
// 	sData += "			</http-param-list>";
// 	sData += "			<content content-type='xml'>";
// 	sData += "				<content-param name='root'>gubun/rpt1/rexdataset/rexrow</content-param>";
// 	sData += "				<content-param name='preservedwhitespace'>1</content-param>";
// 	sData += "				<content-param name='bindmode'>name</content-param>";
// 	sData += "			</content>";
// 	sData += "		</connection>";
