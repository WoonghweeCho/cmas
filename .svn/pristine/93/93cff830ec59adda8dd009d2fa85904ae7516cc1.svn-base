...
//gf_AlertMsg(ds_SVPN.get(i, 'code'));
		$("#SVPN").html($("#SVPN").html() +
/*
		"<input name='privCd' id='privCd' type='radio' value='" + ds_SVPN.get(i, 'code') + "' /><span>" + ds_SVPN.get(i, 'value') + "</span><br>");
작업중 */
		"<input name='privCd' id='privCd' type='radio' value='1' /><span>" + ds_SVPN.get(i, 'value') + "</span><br>");
	}
...
function f_Callback(strSvcId, obj, resultData){
	  ...
	  switch(strSvcId) {
	  	...
	  	case "SELECT_ID_APPN_DTL_SYS_LIST" :
	  		...
	  		//항상 SVPN "사용"으로 표시
	  		for(var i=0; i<ds_IdAppnSysSVPN.size(); i++){
				ds_IdAppnSysSVPN.set(i, "privCd", "1");
			}
/* (나중에 수정)
	  		//원래 종료일자와 비교용
	  		for(var i = 0 ; i < ds_IdAppnDtlList.size(); i++){
