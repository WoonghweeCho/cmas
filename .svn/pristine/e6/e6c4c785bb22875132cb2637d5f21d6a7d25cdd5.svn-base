 ...
 * 업무구분 : 공통모듈
 * 프로그램 : MapFactory
 * 설    명 : 캐시를 구성하기 위한 Map을 지정하기 위해 필요한 초기 사이즈 구성을 위한 데이터를 제공한다.
 *           내부적으로 Primes 값을 근간으로 하여 Map 의 특성을 최대한 이용할 수 있는 Map 사이즈를 산출하여 Map 클래스를 생성한다.
 * 작 성 자 : 
 * 작성일자 : 2012-02-06
 * 수정이력
 * ---------------------------------------------------------------
...
public class MapFactory {
    ...
    private static final int[] PRIMES = { 167, 191, 223, 251, 281, 313, 349, 389, 433, 487, 541, 601, 673, 751, 839, 937, 1049, //
    ...
    private static int getNearPrime(int number) {
        ...
        return PRIMES[(value < 0) ? -(value + 1) : value];
    }
