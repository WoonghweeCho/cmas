...
// viewer Version
var rex_viewer_version = "1,0,0,306";
...
function rex_ParamSet(id) {
	...
	rex_ParamSet.prototype.toString = function() {
		...
		{
			sb += rebfile.toStringFile(this.rptname, "", "", rebfile.toStringConfigParam(null, this.cookie)) + "";
		...
		for(var nkey in this.rebfiles)
		{
			...
			if(this.rebfiles[nkey].rptname != undefined)
			{
				...
				sb += rebfile.toStringFile(this.rebfiles[nkey].rptname, rebfile.toStringField(this.rebfiles[nkey].params), sConn, rebfile.toStringConfigParam(this.rebfiles[nkey].configparams, this.cookie)) + "";
...
function rex_RebFile() {
	...
	rex_RebFile.prototype.toStringConfigParam = function(configparam, cookie) {
		var sb = "";
		...
		if (cookie != "") sb += "<config-param name='cookie'><![CDATA[" + cookie + "]]></config-param>";
	...
	rex_RebConnction.prototype.toString = function(oParam, oHttpParams, oDataSets) {
		...
		if (type == "http") {
			...
			*/
			...
			if (cookie != "") {
				sb += "<config-param name='cookie'><![CDATA[" +  cookie + "]]></config-param>";
			} else {
