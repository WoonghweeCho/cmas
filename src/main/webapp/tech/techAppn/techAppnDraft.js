...
function cf_SetComponents()
{
	...
	//대여일/반납일 검색 Date Component 초기화
	$( "input[name='rentDd']" ).mask("9999-99-99").datepicker({ dateFormat: "yy-mm-dd" });
	//$( "input[name='rtrnDd']" ).mask("9999-99-99").datepicker({ dateFormat: "yy-mm-dd" });
...
function cf_SetEventListener()
{
	...
	$("#btnAppn").click(function(){
		...
		var v_rentDd = gf_GetValue($("input[name='rentDd']").val()); 	//대여일
		var v_rtrnDd = gf_GetValue($( "input[name='rtrnDd']" ).val()); 	//반납일
		var v_subject = datas.subject;
		var fstRegUserId = gv_userId;
		var fnlEditUserId = gv_userId;
		...
		gf_Transaction("DRAFT_TECH_APPN", "/tech/techAppn/insertTechAppnBas.xpl", params, {}, "f_Callback", false);
	});
	...
	$("input[name='rentDd']").bind('change', function() {

		   //$("input[name='rtrnDd']").datepicker("setDate", $("input[name='rentDd']").val());
		   var tmpsDate = $("input[name='rentDd']").val();
		   var stDate = tmpsDate.split("-");
		   var afterDay = "";
		   var dt = new Date(stDate[0], stDate[1]-1, stDate[2]);
		   var dt1 = new Date(stDate[0], stDate[1], stDate[2]);
		   ...
		   if(datas.cls=="DV"){
			   ...
			   afterDay = dt.format('YYYY-MM-DD');

		   }else{
			   afterDay = dt1.format('YYYY-MM-DD');
		   }
		   $("input[name='rtrnDd']").val(afterDay);
	});
...
function f_CheckValidationForSave(){
	...
	var rentDd = gf_GetValue($("input[name='rentDd']").val());	//대여일
	var rtrnDd= gf_GetValue($("input[name='rtrnDd']").val());	//반납일

	if(gf_IsNull(rentDd)){ gf_AlertMsg("대여일은 필수입니다."); return false; };
	if(gf_IsNull(rtrnDd)){ gf_AlertMsg("반납일은 필수입니다."); return false; };
	if(rentDd < curdd){ gf_AlertMsg("대여일은 신청일 이후여야 합니다."); return false; };
}
