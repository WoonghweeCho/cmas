...
var gv_UploadManagerObj = "<div id=\"uploadDiv\">" +
                          ...
                          "		<a class=\"btn s4\" onclick=\"gf_AddFileClick()\" ><span>파일열기</span></a> " + //<input type=\"button\" id=\"btnUploadOpen\"   value=\"열기\" onclick=\"gf_AddFileClick()\"> " +
                          "     <a class=\"btn s4\" onclick=\"gf_DeleteFileClick()\" ><span>파일삭제</span></a> " + //<input type=\"button\" id=\"btnUploadDelete\" value=\"삭제\" onclick=\"gf_DeleteFileClick()\"> " +
                          "     <a class=\"btn s4\" onclick=\"gf_onFileDownload()\" ><span>다운로드</span></a> " +
                          "     <a class=\"btn s4\" onclick=\"gf_onFileInit(true)\" ><span>취소</span></a> " + // <input type=\"button\" id=\"btnUploadCancel\" value=\"취소\" onclick=\"gf_onFileInit()\"> " +
                          " 	</div> " +
						  "		<OBJECT id=\"FileUploadManager\" codeBase=\""+gv_ServerUrl+"/common/install/download/DEXTUploadX.cab#version=3,2,10,0\" height=\"130px\" width=\"100%\" classid=\"CLSID:DF75BAFF-7DD5-4B83-AF5E-692067C90316\" VIEWASTEXT> " +
...
var gv_DownloadManagerObj = "<div id=\"downloadDiv\">" +
							...
							"		<a class=\"btn s4\" onclick=\"gf_onFileDownload()\" ><span>다운로드</span></a> "+ // <input type=\"button\" id=\"btnDownload\" value=\"다운로드\" onclick=\"gf_onFileDownload()\"> " +
							" 	</div> " +
							"	<OBJECT ID=\"FileDownloadManager\" height=\"100%\" width=\"100%\" CodeBase = \""+gv_ServerUrl+"/common/install/download/DEXTUploadX.cab#version=3,2,10,0\" CLASSID=\"CLSID:535AE497-8E85-45F8-AF36-2DFCBCA8B68A\"> " +
							"			<PARAM NAME=\"VisibleContextMenu\" VALUE=\"FALSE\"> " +
							"	</OBJECT> " +
							"   </span> " +
							"</div>" +
...
function f_DownDRMFile() {
	...
	if (document.all('FileDownloadManager').TotalCount == 0 ) {
		// 다운로드 받을 파일이 없습니다.
		gf_AlertMsg("co.inf.notExistDownFile");
		return;
	}

	if ( document.all("FileDownloadManager").SelectedFileCount < 1 ) {
		// 다운로드 받을 파일을 선택하세요.
		gf_AlertMsg('co.warn.selectDownloadFile');
	...
	if ( document.all("FileDownloadManager").SelectedFileCount > 1 ) {
		...
		gf_AlertMsg('co.warn.drmFileOnlyOneDown');
		document.all("FileDownloadManager").UnselectAll();
		return;
	}
	...
	var ServerOrigin 	= "cmas";  								// 전자결재의 경우 cmas
...
//	var downUrl = gv_cmasComm.drmdownUrl+"?UserID="+gv_cmasComm.userId;
	var downUrl = "http://drm.daewooenc.com:1440/Down/Download_dev.asp"+"?UserID="+gv_cmasComm.userId;
...
function f_DextDownload() {
	...
	var winstyle= "height=420,width=445, status=no,toolbar=no,menubar=no,location=no" ;
	window.open(gv_ContextPath+"/common/jsp/comp/fileupload/DextFileDownloadMonitor.jsp", null, winstyle);
}
...
//		alert("1" + fileAtchId + "/" + gv_FileAtchId);
	}
	else {
		gv_FileAtchId = fileAtchId;
//		alert("2" + fileAtchId + "/" + gv_FileAtchId);
	}
	...
	gf_Transaction("SELECT_FILELIST", "/co/common/file/retrieveWebFileList.xpl", fileInfo, {}, "gf_FileList_Callback", false);
}

/**
* @function
* @param  fileAtchId<String> 첨부파일 아이디
...
function gf_retrieveFileListInit(fileAtchId) {
	...
	// 조회 transaction 수행
	var fileInfo =  { "fileAtchId":fileAtchId } ;
	gf_Transaction("SELECT_FILELIST_INIT", "/co/common/file/retrieveWebFileList.xpl", fileInfo, {}, "gf_FileList_Callback", false);
}

/**
* @function
* @param
* @see   첨부파일 ID를 생성하는 함수
* @return N/A
* @author 변형구
* @since 2013-03-20
* @version 1.0
...
function gf_retrieveFileAtchId() {
	...
	if ( gf_IsNull(gv_FileAtchId) ) {
		gf_Transaction("SELECT_FILEATCHID", "/co/common/file/retrieveFileAtchId.xpl?saveYn=Y", {}, {}, "gf_FileList_Callback", false);
	}
}
				...
				if ( i == "fileList") {
					$.each(itemAry, function( j, item)  {
						...
						if ( gv_Mode == "upload" || gv_Mode == "all") {
							document.all('FileUploadManager').AddUploadedFile(item.fileId, item.fileNm, item.fileSize, gv_ServerUrl+"co/common/file/downloadUploadedFile.xpl?fileAtchId="+item.fileAtchId+"&fileId="+item.fileId );
						}

						// downloadmanager data set
						var downUrl = gv_ServerUrl+"co/common/file/downloadUploadedFile.xpl?fileAtchId="+item.fileAtchId+"&fileId="+item.fileId;
...
function gf_AddFileClick() {
	...
	var jsonFileInfo = {"fileAtchId":gv_FileAtchId, "fileId":"", "sysCd":"", "filePath":"", "fileNm":fileObj, "fileSize":fileObj.Size};

	if(jsonFileInfo.fileSize > 20971520){
       	gf_AlertMsg("첨부 가능한 각 파일 크기는 20메가 입니다.\r\n20메가가 넘는 파일은 첨부에서 제외합니다. ");
       	document.all("FileUploadManager").DeleteSelectedFile(document.all("FileUploadManager").Count);
       	return false;
	} else {
...
function gf_AddFileInfoGdsFile(oldCnt, newCnt) {
	...
	for ( var i= 0; i < (newCnt - oldCnt); i++ ) {
		...
		var jsonFileInfo = {"fileAtchId":gv_FileAtchId, "fileId":"", "sysCd":"", "filePath":"", "fileNm":fileObj.name, "fileSize":fileObj.Size};
		gds_FileList.add(jsonFileInfo);
	}
	gv_FileCnt = gds_FileList.size();
}
...
function gf_DeleteFileClick() {
	...
	if ( document.all("FileUploadManager").Count == 0 ) {
		gf_AlertMsg("co.err.noDeleteFile");
		return;
	}
