...
function cf_InitParam()
{
	...
	v_StayStYmd = gf_IsNull(datas.stayStYmd) ? ""          : datas.stayStYmd;
	v_StayEdYmd = gf_IsNull(datas.stayEdYmd) ? ""          : datas.stayEdYmd;
	v_DutyCont = gf_IsNull(datas.dutyCont) ? ""          : datas.dutyCont;
...
function cf_SetEventListener()
{
	...
	$("input[name='stayStYmd']").bind('change', function() {

//		if($("input[name='stayEdYmd']").val() == ""){
//			$("input[name='stayEdYmd']").datepicker("setDate", $("input[name='stayStYmd']").val());
//		}

		if($("input[name='stayStYmd']").val() != "" && $("input[name='stayEdYmd']").val() != ""){
			var sDate = $("input[name='stayStYmd']").val().split("-");
			var eDate = $("input[name='stayEdYmd']").val().split("-");
	...
	// 여권발급일 From To Validation
	$("input[name='stayEdYmd']").bind('change', function() {

//		if($("input[name='stayStYmd']").val() == ""){
//			$("input[name='stayStYmd']").datepicker("setDate", $("input[name='stayEdYmd']").val());
//		}
...
function cf_InitForm()
{
	if(v_NatCd == ""){
		...
		$("input[name='stayStYmd']").val(v_StayStYmd);
		$("input[name='stayEdYmd']").val(v_StayEdYmd);
		$("#selectedNat").text(v_NatNm);

		$("#dutyCont").val(v_DutyCont);
...
function f_Confirm() {
	...
	if($("input[name='stayStYmd']").val() == ""){
		gf_AlertMsg("체류 시작일을 입력해주세요.");
		return;
	}
	if($("input[name='stayEdYmd']").val() == ""){
		gf_AlertMsg("체류 종료일을 입력해주세요.");
		return;
	}

	if($("#dutyCont").val() == ""){
		gf_AlertMsg("주요업무내용을 입력해주세요.");
		return;
	}
	...
	var callbackObj = {
			...
			natNm : v_NatNm,
			cityNm : $("#cityName").val(),
			stayStYmd : $("input[name='stayStYmd']").val(),
			stayEdYmd : $("input[name='stayEdYmd']").val(),
			riskYn : v_RiskYn,
