 ...
 * 데이터를 형변환하기 위한 클래스이다.
 *
 * 임의의 데이타를 다른 타입의 데이타로 변환하기 위해서 사용된다.
 * 중요한 특징으로 Null값을 값을 반환하지 않는다.
 * 어떠한 형이던지 다른 임의의 형으로 변환할 수 있으며 단지.
 * 의미적으로 "abc"문자열을 숫자로 변환하면 무조건 0을 리턴한다.
 * 그러나 "123"은 자동으로 맞춰준다.
 * 단 입력 포맷에 주의할 것은 날짜관련 데이타인데...
 * 문자열을 Date형태로 변환하고자할 경우에
 *
...
public class CastUtil {
    ...
    /**
     * Cast도중에 에러가 발생하면 default로 값을 변환하여 리턴한다.
     * @param value
     */
...
/**
     * noDefault = true이면
     * 입력 값에 대한 Conversion시 에러가 발생하면 default로 변화하지 않고
    ...
    /**
    * 입력값을 int로 변환하여 반환한다.
    * 만약 변환중 에러가 발생하거나 null이면 0을 리턴한다.
     * 단)생성자에서 noDefault = true이면서 에러이면 XRuntimeException
    * @return int
    */
    public int cint() {
        int out = 0;
     ...
     * 만약 변환중 에러가 발생하거나 null이면 new Integer(0)을 리턴한다.
     * 단)생성자에서 noDefault = true이면서 에러이면 XRuntimeException
      * @return Integer
     */
    ...
    /**
     * 입력값을 Long으로 변환하여 반환한다.
     * 만약 변환중 에러가 발생하거나 null이면 new Long(0)을 리턴한다.
     * 단)생성자에서 noDefault = true이면서 에러이면 XRuntimeException
     * @return Long
     */
    public Long cLong() {
    ...
    /**
     * 입력값을 Float로 변환하여 반환한다.
     * 만약 변환중 에러가 발생하거나 null이면 new Float(0)을 리턴한다.
      * 단)생성자에서 noDefault = true이면서 에러이면 XRuntimeException
    * @return Float
     */
    public Float cFloat() {
    ...
    /**
     * 입력값을 Double로 변환하여 반환한다.
     * 만약 변환중 에러가 발생하거나 null이면 new Double(0)을 리턴한다.
     * 단)생성자에서 noDefault = true이면서 에러이면 XRuntimeException
     * @return Double
     */
    public Double cDouble() {
     ...
     * 만약 변환중 에러가 발생하거나 null이면 new BigDecimal(0)을 리턴한다.
       * 단)생성자에서 noDefault = true이면서 에러이면 XRuntimeException
    * @return BigDecimal
     */
     ...
     * 만약 변환중 에러가 발생하거나 null이면 ""을 리턴한다.
      * 단)생성자에서 noDefault = true이면서 에러이면 XRuntimeException
    * @return String
     */
     ...
     * 만약 변환중 에러가 발생하거나 null이면 new java.sql.Date(0)을 리턴한다.
     * 단)생성자에서 noDefault = true이면서 에러이면 XRuntimeException
     * @return Date
     */
     ...
     * 만약 변환중 에러가 발생하거나 null이면 new java.sql.Time(0)을 리턴한다.
     * 단)생성자에서 noDefault = true이면서 에러이면 XRuntimeException
     * @return Time
     */
     ...
     * 만약 변환중 에러가 발생하거나 null이면 new java.sql.Timestamp(0) 을 리턴한다.
     * 단)생성자에서 noDefault = true이면서 에러이면 XRuntimeException
     * @return Timestamp
     */
    ...
    public Boolean cBoolean() {
        if (value instanceof String) {
            return new Boolean("true".equalsIgnoreCase((String) value));
        } else if (value instanceof Boolean) {
